...
function renderNewsBlocks(blocks) {
    return (blocks||[]).map(b => {
        ...
        if (b.type === 'video') {
            ...
            if (isYt) {
                let embed = url;
                embed = embed.replace('watch?v=','embed/').replace('youtu.be/','www.youtube.com/embed/');
                return `<div class="my-4"><div class="relative w-full" style="padding-bottom:56.25%"><iframe src="${embed}" class="absolute top-0 left-0 w-full h-full rounded-xl border border-slate-800" allowfullscreen loading="lazy"></iframe></div>${b.caption?`<p class="text-xs text-slate-500 mt-2 text-center">${b.caption}</p>`:''}</div>`;
            }
            return `<div class="my-4"><video controls class="w-full rounded-xl border border-slate-800 bg-black max-h-96"><source src="${url}">Ваш браузер не підтримує відео.</video>${b.caption?`<p class="text-xs text-slate-500 mt-2 text-center">${b.caption}</p>`:''}</div>`;
        }
        if (b.type === 'html')
            return `<div class="my-3 text-sm text-slate-300 leading-relaxed">${b.value||''}</div>`;
        if (b.type === 'divider')
